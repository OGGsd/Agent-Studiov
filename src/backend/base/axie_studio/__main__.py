...
from axie_studio.cli.progress import create_langflow_progress
from axie_studio.initial_setup.setup import get_or_create_default_folder
from axie_studio.logging.logger import configure, logger
from axie_studio.main import setup_app
from axie_studio.services.database.utils import session_getter
from axie_studio.services.deps import get_db_service, get_settings_service, session_scope
from axie_studio.services.settings.constants import DEFAULT_SUPERUSER
from axie_studio.services.utils import initialize_services
from axie_studio.utils.version import fetch_latest_version, get_version_info
from axie_studio.utils.version import is_pre_release as langflow_is_pre_release
...
def run(
    *,
    ...
    if platform.system() == "Windows":
        ...
        with progress.step(6):
            # Use Gunicorn with AxieStudioUvicornWorker for non-Windows systems
            from axie_studio.server import AxieStudioApplication
            ...
            server = AxieStudioApplication(app, options)
...
def print_banner(host: str, port: int, protocol: str) -> None:
    ...
    styled_package_name = stylize_text(
        "Axie Studio", "Axie Studio", is_prerelease=any("pre-release" in notice for notice in notices)
    )
    ...
    access_link = f"[bold]{status_icon} Open Axie Studio {arrow}[/bold] [link={protocol}://{access_host}:{port}]{protocol}://{access_host}:{port}[/link]"
    ...
    try:
        ...
        try:
            ...
            logger.info("Welcome to Axie Studio")
            logger.info(f"Open Axie Studio: {protocol}://{access_host}:{port}")
...
def superuser(
    ...
    async def _create_superuser():
        ...
        async with session_getter(db_service) as session:
            from axie_studio.services.auth.utils import create_super_user
    ...
    async def aapi_key():
        ...
        async with session_scope() as session:
            ...
            if not superuser:
                ...
                return None
            from axie_studio.services.database.models.api_key.crud import create_api_key, delete_api_key
            from axie_studio.services.database.models.api_key.model import ApiKey, ApiKeyCreate
